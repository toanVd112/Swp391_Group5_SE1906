...
public class ManageRoomList {
     ...
     *  - roomTypeId (nullable)
     *  - status (nullable)
     *  - số phòng chứa keyword (nullable)
     *  - floor from-to (nullable)
     *  - price from-to (nullable)
     */
    public List<Room2> searchRooms(
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            // 1-2: roomTypeId
            ps.setObject(1, roomTypeId);  ps.setObject(2, roomTypeId);
            // 3-4: status
            ps.setObject(3, status);      ps.setObject(4, status);
            // 5-6: keyword
            ps.setObject(5, keyword);     ps.setObject(6, keyword);
            // 7-8: minFloor
            ps.setObject(7, minFloor);    ps.setObject(8, minFloor);
            // 9-10: maxFloor
            ps.setObject(9, maxFloor);    ps.setObject(10, maxFloor);
            // 11-12: minPrice
            ps.setObject(11, minPrice);   ps.setObject(12, minPrice);
            // 13-14: maxPrice
            ps.setObject(13, maxPrice);   ps.setObject(14, maxPrice);
